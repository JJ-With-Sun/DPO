...
def return_prompt_and_responses(samples):
  if samples['input'] == "":
    samples['text'] = "[INST]" + samples['instruction'] + "[/INST]" + samples['output']
  else:
    samples['text'] = "<s>[INST]"  + samples['instruction'] + samples['input'] + "[/INST]" + samples['output']
  return samples

def return_prompt_and_chosen_rejected(samples):
   samples['prompt']= samples['instruction_A']
   samples['chosen']= samples['input_A']+samples['output_A']
   samples['rejected']= samples['input_B']+samples['output_B']
   return samples
...
def load_and_prepare_dataset(dataset_path, types):
    ...
    if types == "lora_sftt":
      ...
      original_columns = dataset.column_names
      dataset = dataset.map(
        return_prompt_and_chosen_rejected,
        remove_columns=original_columns
      )
      dataset = dataset.map(
